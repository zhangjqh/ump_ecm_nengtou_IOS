...
//  Copyright (c) yonyou. All rights reserved.
//
...
+ (MaHomeView *)defaultButtonBoard{
    
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^{
        __board = [[MaHomeView alloc] init];
    });
    return __board;
}
...
- (id)init{
    ...
    if (self) {
        ...
        _boardWindow = [[UIWindow alloc] initWithFrame:CGRectMake(0, 0, 50, 50)];
        ...
        self.boardImage = [UIImage imageNamed:@"MaHomeView1.png"];
        self.boardSize = 45 ;
        self.buttonNumber = 2;
        self.viewTransparency = 0.3;
        self.highlightedTime = 2.0;
        CGPoint position = CGPointMake(22.5,200);
        [self setBoardPosition:position animate:NO];
        
        NSArray *imgArray = [NSArray arrayWithObjects:[UIImage imageNamed:@"MaHomeView2.png"],
                             [UIImage imageNamed:@"MaHomeView3.png"],nil];
        self.buttonImageArray = imgArray;
...
- (void)setViewTransparency:(CGFloat)viewTransparency
{
    _viewTransparency = viewTransparency ;
}

- (void)setHighlightedTime:(CGFloat)highlightedTime
{
    _highlightedTime = highlightedTime ;
    [self performSelector:@selector(updateTime) withObject:nil afterDelay:highlightedTime] ;
}

- (void)update
{
    [self performSelector:@selector(updateTime) withObject:nil afterDelay:2] ;
}

- (void)updateTime
{
    _boardView.alpha = _viewTransparency ;
}
...
- (void)boardOpen{
    ...
    for (int i = 0; i < _buttonNumber; i++) {
        ...
        [_boardWindow bringSubviewToFront:button];
    ...
    [UIView animateWithDuration:0.3
                     ...
                     completion:^(BOOL finished) {
                         ...
                         _animating = NO;
                         _isOpen = YES;
                         UIButton *button = [[UIButton alloc] initWithFrame:_boardWindow.bounds];
                         [button addTarget:self action:@selector(windowTaped:) forControlEvents:UIControlEventTouchUpInside];
                         [_boardWindow addSubview:button];
