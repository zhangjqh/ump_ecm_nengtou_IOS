...
@implementation WAAnimatedGifFrame

@synthesize data, delay, disposalMethod, area, header;

@end

@implementation CWAGIFImageView
...
{
  CWAGIFImageView *gifImageView = [CWAGIFImageView  gifImageViewWithGIFData:gifImageData];
  if (!gifImageView)
  {
    return nil;
  }
...
{
  NSData* imageData = [NSData dataWithContentsOfFile:gifFilePath];
  return [CWAGIFImageView gifImageViewWithGIFData:imageData];
}

+ (CWAGIFImageView *)gifImageViewWithGIFData:(NSData*)gifImageData
{
  ...
  {
    CWAGIFImageView *gifImageView = [[CWAGIFImageView alloc] initWithGIFData:gifImageData];
    return gifImageView;
  }
}
...
{
  if (![CWAGIFImageView isGifImage:gifImageData])
  {
...
- (void)loadImageData 
{
  ...
  int i = 0;
  WAAnimatedGifFrame *lastFrame = nil;
  for (UIImage *image in array)
  {
    // Get Frame
    WAAnimatedGifFrame *frame = [GIF_frames objectAtIndex:i];
	// Initialize Flag
  ...
  double total = 0;
  for (WAAnimatedGifFrame *frame in GIF_frames) 
  {
    total += frame.delay;
  }
  // GIFs store the delays as 1/100th of a second,
  // UIImageViews want it in seconds.
  [self setAnimationDuration:total/40];
 ...
 [GIF_buffer getBytes:cur length:1];
	...
	while (cur[0] != 0x00)
  {
		...
		if (cur[0] == 0x04 && prev[0] == 0xF9)
		{
			...
			WAAnimatedGifFrame *frame = [[WAAnimatedGifFrame alloc] init];
...
- (void) GIFReadDescriptor {
	...
	WAAnimatedGifFrame *frame = [GIF_frames lastObject];
	frame.area = rect;
...
- (NSData*) getFrameAsDataAtIndex:(int)index {
	...
	{
		return ((WAAnimatedGifFrame *)[GIF_frames objectAtIndex:index]).data;
	}
	else
	{
		return nil;
