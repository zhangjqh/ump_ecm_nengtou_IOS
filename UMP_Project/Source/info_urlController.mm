...
static NSString *controllerC = @"info_urlController";
...
- (void)initChilds
{
    ...
    [(UMLayoutView*)_info_url addSubUMView:_viewPage0];
     //设置控件基本属性
...
[self umcontrol:_webview0 controlid:@"webview0" bindfield:@"url"];
...
- (void)htmlBind:(id<UMViewControl>)sender args:(XEventArgs *)args{
if(args==nil) {
  args = [[XEventArgs alloc] init:self];
}
[args setInvoke:[self getInvokeInfo:@"htmlBind" method:@"UMService.transInfoService" sender:sender]];
...
[args setInvoke:[self getInvokeInfo:@"infoLoad" method:@"UMService.callAction" sender:sender]];
 ...
 [args putValue:@"oneUrlLoad" forKey:@"action"];
