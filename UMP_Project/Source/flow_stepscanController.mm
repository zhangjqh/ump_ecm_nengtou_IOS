...
#import "flow_stepscanController.h"
...
@interface flow_stepscanController ()
{
...
static NSString *controllerC = @"StepScanController";
...
- (void)initChilds
{
    ...
    [(UMLayoutView*)_StepScan addSubUMView:_basePanel];
     //设置控件基本属性
...
// --navLabel
    _navLabel = [UMControl creatControl:@"UMXLabel" container:self  params:@{
                          ...
                          @"txt_b":@"0.0",
                          @"valign":@"center",
                          @"readonly":@"true",@"value":@"#{res.flow_processview}"}];
...
- (void)gotoPerson:(id<UMViewControl>)sender args:(XEventArgs *)args{
if(args==nil) {
  args = [[XEventArgs alloc] init:self];
}
[args setInvoke:[self getInvokeInfo:@"gotoPerson" method:@"UMView.open" sender:sender]];

 [args putValue:@"#{CurrentPerson}" forKey:@"currentperson"];

 [args putValue:@"ecm_personinfo" forKey:@"viewid"];
...
[args setInvoke:[self getInvokeInfo:@"onItemClick" method:@"UMService.callAction" sender:sender]];
 ...
 [args putValue:@"#{#{cursor.cursoreditstep_stepsalias}.id}" forKey:@"personid"];

 [args putValue:@"gotoPerson" forKey:@"callback"];

 [args putValue:@"info" forKey:@"action"];

 [args putValue:@"false" forKey:@"autodatabinding"];

 [args putValue:@"information" forKey:@"type"];

 [args putValue:@"CurrentPerson" forKey:@"contextmapping"];

 [args putValue:@"editstep.steps" forKey:@"personid_arrayPath"];
