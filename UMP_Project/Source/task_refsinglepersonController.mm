...
@interface task_refsinglepersonController ()
{
...
@implementation task_refsinglepersonController
@synthesize navBarHidden = _navBarHidden;
@synthesize viewObject = _viewObject;
@synthesize refsinglepersonwin = _refsinglepersonwin;
...
static NSString *controllerC = @"refsinglepersonController";
...
- (id)initWithParentCtl:(UIViewController*)parentCtl
{
    ...
    if (self) {
        ...
        _refsinglepersonwin = _viewObject;
...
- (void)initChilds
{
    ...
    [(UMLayoutView*)_refsinglepersonwin addSubUMView:_viewPage0];
     //设置控件基本属性
...
[args setInvoke:[self getInvokeInfo:@"onClickListRow" method:@"UMView.close" sender:sender]];
 ...
 [args putValue:@"#{#{cursor.cursorstafflistalias}}" forKey:@"result"];

 [args putValue:@"stafflist" forKey:@"result_arrayPath"];
...
- (void)searchpsnbykey:(id<UMViewControl>)sender args:(XEventArgs *)args{
if(args==nil) {
  args = [[XEventArgs alloc] init:self];
}
[args setInvoke:[self getInvokeInfo:@"searchpsnbykey" method:@"searchpsnbykey" sender:sender]];
[UMCommonSevice callSeviceWithMethod:args];
