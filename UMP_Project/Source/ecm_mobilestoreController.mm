...
#import "ecm_mobilestoreController.h"
...
@interface ecm_mobilestoreController ()
{
...
@implementation ecm_mobilestoreController
@synthesize navBarHidden = _navBarHidden;
@synthesize viewObject = _viewObject;
@synthesize Ecm_Mobilestore = _Ecm_Mobilestore;
...
static NSString *controllerC = @"Ecm_MobilestoreController";
...
- (void)initChilds
{
    ...
    [(UMLayoutView*)_Ecm_Mobilestore addSubUMView:_viewPage0];
     //设置控件基本属性
    ...
    _navigatorbar0.naviTitle = [UMParser parseExpr:nil expr:@"#{res.ecm_mobilestore}" ctx:_ctx];;
...
// --webview0
    _webview0 = [UMControl creatControl:@"UMXWebView" container:self  params:@{
                          ...
                          @"layout":@"Layout_vbox",
                          @"supportzoom":@"false",
                          @"url":@"http://app.yonyou.com",@"value":@""}];
    [(UMLayoutView*)_panel0 addSubUMXView:_webview0];
...
- (void)closeMobileStore:(id<UMViewControl>)sender args:(XEventArgs *)args{
if(args==nil) {
  args = [[XEventArgs alloc] init:self];
}
[args setInvoke:[self getInvokeInfo:@"closeMobileStore" method:@"UMView.close" sender:sender]];
